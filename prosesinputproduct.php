    ...
    if ($statusUploud == 0) {
        ...
        if (mysqli_num_rows($select) > 0) {
            ...
            if (move_uploaded_file($_FILES['gambar']['tmp_name'], $target_file)) {
                $query = mysqli_query($conn, "INSERT INTO produk (nama_produk, harga, keterangan, stok, gambar) values ('$nama_produk', '$harga', '$keterangan', '$stok', '" . $kode_rand . $_FILES['gambar']['name'] . "')");
                if ($query) {
